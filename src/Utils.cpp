...
void splitCmd(const std::string& rawline,
              std::string& command,
    ...
    std::string head = (colon == std::string::npos) ? line.substr(pos) : line.substr(pos, colon - pos);
    if (colon != std::string::npos) trailing = line.substr(colon + 2);
